...
const border = "3px black solid"

const DisplayData = ({data}) => 
<div style={{border: `${border}`}}>
  {data.map((items, index) => 
  <div key={index}>
    <p>Nation: {items.Nation}</p>
    <p>Year: {items.Year}</p>
...
const DisplaySwapi = ({swapi, border}) =>
<div style={{border: `${border}`}}>
  {swapi.map((items, index) => <div key={index} style={{border: `${border}`}}>
    <p>Name: {items.name}</p>
    <p>Height: {items.height}</p>
    <p>Mass: {items.mass}</p>
    <p>Hair Color: <span style={{color: items.hair_color}}>{items.hair_color === "n/a" || items.hair_color === "none" || items.hair_color}</span></p>
    <p>Birth Year: {items.birth_year}</p>
    <p>Gender: {items.gender === "n/a" || items.gender}</p>
  </div> )}
</div>
...
<div>
    ...
    {episodes.map((items,index)=> <div key={index}>
        <h4>Episode {index+1}</h4>
        <p>Name episode: <em>{items.name}</em></p>
        <p>Relese date: {items.air_date}</p>
        <div>Characters: 
            <ul>
                {items.characters.map((items, index) =>
...
<div>
    ...
    <div key={index} style={{border}}>
        <p>Character name: <em>{items.name}</em></p>
        <p>Gender: {items.gender}</p>
        <img src={items.image}></img>
        <div>
            Episode:
            <ul>
                {items.episode.map((items, index) =>
                <li key={index}>{items.name}, {items.air_date}</li>
                )}
...
function App() {
  return (
    <div>
      ...
      <DisplaySwapi swapi={swapi} border={border}/>
      <DisplayEpisodes episodes={episodes}/>
      <DisplayCharactes characters={characters}/>
    </div>
